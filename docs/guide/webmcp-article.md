...
*   **深度不可感知**：AI 只能看到“表面”，无法感知网页背后的隐藏状态（如未展示在 UI 上的后端元数据）。
...
对于复杂的异步交互或非表单类业务，可以通过 `navigator.modelContextTesting` 进行动态绑定：

*   **业务端注册示例**：
    ```javascript
    navigator.modelContextTesting.registerTool({
...
- [OpenTiny 兼容性协议方案讨论](https://github.com/opentiny/webmcp-sdk/discussions/393)
