...
如果需要手动更新模型配置，可以通过组件暴露的 `agent` 实例来访问底层 Provider。但通常情况下，通过 `v-model:selectedModelId` 切换模型已经足够，组件会自动处理所有更新逻辑。
...
watch(selectedModelId, async (newModelId) => {
  ...
  // 可以通过 agent 访问底层 Provider（通常不需要手动操作）
  const agent = remoterRef.value?.agent
  agent.updateLLMConfig({
    modelId: newModelId,
    baseURL: modelConfigs.find((model) => model.id === newModelId)?.baseURL || '',
    apiKey: modelConfigs.find((model) => model.id === newModelId)?.apiKey || '',
    providerType: modelConfigs.find((model) => model.id === newModelId)?.providerType || '',
    useReActMode: modelConfigs.find((model) => model.id === newModelId)?.useReActMode || false
  })
})
</script>
```
