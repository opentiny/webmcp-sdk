...
本文档介绍如何在 `TinyRemoter` 组件中配置和使用 Skills 技能系统。
...
当用户提问时，AI 会先读取 `SKILL.md` 了解技能概况，再按需加载参考资料文件，从而给出精准回答。
...
- 确认 YAML Front Matter 中的 `description` 字段已填写且描述清晰——AI 依赖此字段判断何时激活该技能
- 检查 glob 路径是否正确，应使用 `./skills/**/*` 而非 `./skills/**/SKILL.md`
