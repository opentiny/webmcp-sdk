...
其核心技术架构基于**浏览器内置的 WebMCP 协议**愿景，通过 **Polyfill** 技术在页面中注入标准化的 `document.modelContext` 接口。这使得开发者能够使用标准的 WebMCP 规范来定义工具，实现了从底层 DOM 操作到高层业务逻辑工具化的全面覆盖。
...
插件深度支持 **WebMCP** 规范，允许 Web 应用在页面内部直接通过 `document.modelContext` 注册业务工具。
