...
> **重要提示**：我们的 Skills 系统完全遵循**业界标准的 Skills 协议规范**。你可以使用标准的 `SKILL.md` 格式定义技能，无需任何显式唤起（如输入 `@`），AI 助手会自动识别用户意图并调用合适的技能来解决问题。

## 概述

Skills（技能）是 AI Extension 赋予大模型专业知识的核心机制。通过采用**渐进式披露（Progressive Disclosure）**设计，避免了一次性给大模型塞入所有 API 约束导致 Token 爆炸或产生幻觉。

具体做法是：

- 系统仅在初始上下文中透出所有技能的"大纲"。
- AI 分析用户意图后，会通过 `view_file` 自主查阅特定技能的详细开发守则和参考资料（按需读取）。

## 界面添加自定义技能（普通用户）

除了使用内置技能，普通用户也可以直接在扩展界面中添加和管理自定义技能，无需修改代码。

### 操作步骤

1. **打开 Skill 管理**：右击浏览器工具栏的 AI Extension 图标，点击「选项」，切换到 Skill 管理页签，点击「添加 Skill」按钮并输入技能名称。
2. **编写技能内容**：在技能列表中找到刚创建的技能，点击「编辑」，按照 Skill 协议格式填写技能名称、描述和提示词（格式与开发者一致，见下方 [SKILL.md 示例](#skill-md-example)）。
3. **保存即生效**：保存后技能立即可用，AI 会自动识别用户意图并在合适场景调用该技能。

> **提示**：界面添加的技能与代码内置的技能效果完全一致，区别仅在于来源——前者由用户在运行时维护，后者由开发者在工程中预置。
...
### 2. 创建 SKILL.md 入口文件 {#skill-md-example}
