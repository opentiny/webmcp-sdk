...
program
  ...
  .action(async (options) => {
    try {
      const result = await stateCommand({ tabid: options.tabid ? parseInt(options.tabid, 10) : undefined })
      console.log(JSON.stringify(result, null, 2))
    } catch (error: any) {
      console.error(pc.red(`Error executing state command: ${error.message}`))
      process.exit(1)
    }
  })
  ...
  .action(async (toolName, argsJson, options) => {
    try {
      const result = await runCommand({
        toolName,
        argsJson,
        tabid: options.tabid ? parseInt(options.tabid, 10) : undefined
      })
      console.log(JSON.stringify(result, null, 2))
    } catch (error: any) {
      console.error(pc.red(`Error executing run command: ${error.message}`))
      process.exit(1)
    }
  })
...
program
  ...
  .action(async (url, options) => {
    try {
      ...
      console.error(pc.red(`Error executing open command: ${error.message}`))
      process.exit(1)
    }
  })
