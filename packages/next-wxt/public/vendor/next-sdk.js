;(function (global2, factory) {
  ...
  util$7.mergeEvaluated = {
    props: makeMergeEvaluated({
      mergeNames: (gen, from, to) =>
        gen.if((0, codegen_1$v._)`${to} !== true && ${from} !== undefined`, () => {
          gen.if(
            (0, codegen_1$v._)`${from} === true`,
            () => gen.assign(to, true),
            () =>
              gen.assign(to, (0, codegen_1$v._)`${to} || {}`).code((0, codegen_1$v._)`Object.assign(${to}, ${from})`)
          )
        }),
      mergeToName: (gen, from, to) =>
        gen.if((0, codegen_1$v._)`${to} !== true`, () => {
          if (from === true) {
            gen.assign(to, true)
          } else {
    ...
    items: makeMergeEvaluated({
      mergeNames: (gen, from, to) =>
        gen.if((0, codegen_1$v._)`${to} !== true && ${from} !== undefined`, () =>
          gen.assign(to, (0, codegen_1$v._)`${from} === true ? true : ${to} > ${from} ? ${to} : ${from}`)
        ),
      mergeToName: (gen, from, to) =>
        gen.if((0, codegen_1$v._)`${to} !== true`, () =>
          gen.assign(to, from === true ? true : (0, codegen_1$v._)`${to} > ${from} ? ${to} : ${from}`)
        ),
  ...
  function getErrorPath(dataProp, dataPropType, jsPropertySyntax) {
    if (dataProp instanceof codegen_1$v.Name) {
      ...
      return jsPropertySyntax
        ...
        : isNumber
          ? (0, codegen_1$v._)`"/" + ${dataProp}`
          : (0, codegen_1$v._)`"/" + ${dataProp}.replace(/~/g, "~0").replace(/\\//g, "~1")`
    }
  ...
  function coerceAndCheckDataType(it, types2) {
    ...
    const checkTypes =
      types2.length > 0 &&
      !(coerceTo.length === 0 && types2.length === 1 && (0, applicability_1$1.schemaHasRulesForType)(it, types2[0]))
    if (checkTypes) {
  ...
  function coerceData(it, types2, coerceTo) {
    ...
    if (opts.coerceTypes === 'array') {
      gen.if((0, codegen_1$s._)`${dataType2} == 'object' && Array.isArray(${data2}) && ${data2}.length == 1`, () =>
        gen
          .assign(data2, (0, codegen_1$s._)`${data2}[0]`)
          .assign(dataType2, (0, codegen_1$s._)`typeof ${data2}`)
    ...
    function coerceSpecificType(t) {
      switch (t) {
        case 'string':
          gen
            .elseIf((0, codegen_1$s._)`${dataType2} == "number" || ${dataType2} == "boolean"`)
            .assign(coerced, (0, codegen_1$s._)`"" + ${data2}`)
            .elseIf((0, codegen_1$s._)`${data2} === null`)
            .assign(coerced, (0, codegen_1$s._)`""`)
          return
        case 'number':
          gen
            .elseIf(
              (0, codegen_1$s._)`${dataType2} == "boolean" || ${data2} === null
        ...
        case 'boolean':
          gen
            .elseIf((0, codegen_1$s._)`${data2} === "false" || ${data2} === 0 || ${data2} === null`)
            .assign(coerced, false)
            .elseIf((0, codegen_1$s._)`${data2} === "true" || ${data2} === 1`)
            .assign(coerced, true)
          return
        case 'null':
        ...
        case 'array':
          gen
            .elseIf(
              (0, codegen_1$s._)`${dataType2} === "string" || ${dataType2} === "number"
  ...
  function assignParentData({ gen, parentData, parentDataProperty }, expr) {
    gen.if((0, codegen_1$s._)`${parentData} !== undefined`, () =>
      gen.assign((0, codegen_1$s._)`${parentData}[${parentDataProperty}]`, expr)
    )
  }
  function checkDataType$1(dataType2, data2, strictNums, correct = DataType.Correct) {
    ...
    function numCond(_cond = codegen_1$s.nil) {
      return (0, codegen_1$s.and)(
        (0, codegen_1$s._)`typeof ${data2} == "number"`,
        _cond,
        strictNums ? (0, codegen_1$s._)`isFinite(${data2})` : codegen_1$s.nil
      )
    }
  }
  ...
  function checkDataTypes$1(dataTypes, data2, strictNums, correct) {
    ...
    for (const t in types2) cond = (0, codegen_1$s.and)(cond, checkDataType$1(t, data2, strictNums, correct))
    return cond
  }
  ...
  function callValidateCode(
    ...
    return context !== codegen_1$q.nil
      ? (0, codegen_1$q._)`${func}.call(${context}, ${args})`
      : (0, codegen_1$q._)`${func}(${args})`
  }
  ...
  function usePattern({ gen, it: { opts } }, pattern2) {
    ...
    return gen.scopeValue('pattern', {
      ...
      ref: rx,
      code: (0,
      codegen_1$q._)`${regExp.code === 'new RegExp' ? newRegExp : (0, util_2$1.useFunc)(gen, regExp)}(${pattern2}, ${u})`
    })
  }
  ...
  function funcKeywordCode(cxt, def2) {
    ...
    function validateAsync() {
      ...
      gen.try(
        () => assignValid((0, codegen_1$p._)`await `),
        (e) =>
          gen.assign(valid, false).if(
            (0, codegen_1$p._)`${e} instanceof ${it.ValidationError}`,
            () => gen.assign(ruleErrs, (0, codegen_1$p._)`${e}.errors`),
            () => gen.throw(e)
          )
      )
      return ruleErrs
    ...
    function assignValid(_await = def2.async ? (0, codegen_1$p._)`await ` : codegen_1$p.nil) {
      ...
      gen.assign(
        valid,
        (0, codegen_1$p._)`${_await}${(0, code_1$9.callValidateCode)(cxt, validateRef, passCxt, passSchema)}`,
        def2.modifying
      )
    }
  ...
  function addErrs(cxt, errs) {
    const { gen } = cxt
    gen.if(
      (0, codegen_1$p._)`Array.isArray(${errs})`,
      () => {
        gen
          .assign(
            names_1$4.default.vErrors,
            (0,
            codegen_1$p._)`${names_1$4.default.vErrors} === null ? ${errs} : ${names_1$4.default.vErrors}.concat(${errs})`
          )
  ...
  function useKeyword(gen, keyword2, result) {
    ...
    return gen.scopeValue(
      'keyword',
      typeof result == 'function' ? { ref: result } : { ref: result, code: (0, codegen_1$p.stringify)(result) }
    )
  }
  ...
  function validateKeywordUsage({ schema, opts, self: self2, errSchemaPath }, def2, keyword2) {
    ...
    if (
      deps === null || deps === void 0 ? void 0 : deps.some((kwd) => !Object.prototype.hasOwnProperty.call(schema, kwd))
    ) {
    ...
    if (keyword2 !== void 0) {
      ...
      return schemaProp === void 0
        ...
        : {
            ...
            schemaPath: (0,
            codegen_1$o._)`${it.schemaPath}${(0, codegen_1$o.getProperty)(keyword2)}${(0, codegen_1$o.getProperty)(schemaProp)}`,
  ...
  function extendSubschemaData(
    ...
    if (dataProp !== void 0) {
      ...
      codegen_1$o.str)`${errorPath}${(0, util_1$n.getErrorPath)(dataProp, dpType, opts.jsPropertySyntax)}`
  ...
  function validateFunction({ gen, validateName, schema, schemaEnv, opts }, body) {
    if (opts.code.es5) {
      gen.func(
        validateName,
        (0, codegen_1$n._)`${names_1$3.default.data}, ${names_1$3.default.valCxt}`,
        schemaEnv.$async,
        () => {
      ...
      gen.func(
        validateName,
        (0, codegen_1$n._)`${names_1$3.default.data}, ${destructureValCxt(opts)}`,
        schemaEnv.$async,
  ...
  function destructureValCxt(opts) {
    return (0,
    codegen_1$n._)`{${names_1$3.default.instancePath}="", ${names_1$3.default.parentData}, ${names_1$3.default.parentDataProperty}, ${names_1$3.default.rootData}=${names_1$3.default.data}${opts.dynamicRef ? (0, codegen_1$n._)`, ${names_1$3.default.dynamicAnchors}={}` : codegen_1$n.nil}}={}`
  }
  function destructureValCxtES5(gen, opts) {
    gen.if(
      names_1$3.default.valCxt,
      () => {
        gen.var(
          names_1$3.default.instancePath,
          (0, codegen_1$n._)`${names_1$3.default.valCxt}.${names_1$3.default.instancePath}`
        )
        gen.var(
          names_1$3.default.parentData,
          (0, codegen_1$n._)`${names_1$3.default.valCxt}.${names_1$3.default.parentData}`
        )
        gen.var(
          names_1$3.default.parentDataProperty,
          (0, codegen_1$n._)`${names_1$3.default.valCxt}.${names_1$3.default.parentDataProperty}`
        )
        gen.var(
          names_1$3.default.rootData,
          (0, codegen_1$n._)`${names_1$3.default.valCxt}.${names_1$3.default.rootData}`
        )
        if (opts.dynamicRef)
          gen.var(
            names_1$3.default.dynamicAnchors,
            (0, codegen_1$n._)`${names_1$3.default.valCxt}.${names_1$3.default.dynamicAnchors}`
          )
      },
      () => {
  ...
  function resetEvaluated(it) {
    ...
    gen.if((0, codegen_1$n._)`${it.evaluated}.dynamicProps`, () =>
      gen.assign((0, codegen_1$n._)`${it.evaluated}.props`, (0, codegen_1$n._)`undefined`)
    )
    gen.if((0, codegen_1$n._)`${it.evaluated}.dynamicItems`, () =>
      gen.assign((0, codegen_1$n._)`${it.evaluated}.items`, (0, codegen_1$n._)`undefined`)
    )
  }
  function funcSourceUrl(schema, opts) {
    const schId = typeof schema == 'object' && schema[opts.schemaId]
    return schId && (opts.code.source || opts.code.process)
      ? (0, codegen_1$n._)`/*# sourceURL=${schId} */`
      : codegen_1$n.nil
  }
  ...
  function returnResults(it) {
    ...
    if (schemaEnv.$async) {
      gen.if(
        (0, codegen_1$n._)`${names_1$3.default.errors} === 0`,
        () => gen.return(names_1$3.default.data),
        () => gen.throw((0, codegen_1$n._)`new ${ValidationError2}(${names_1$3.default.vErrors})`)
      )
    } else {
  ...
  function assignEvaluated({ gen, evaluated, props, items: items2 }) {
    ...
    if (items2 instanceof codegen_1$n.Name) gen.assign((0, codegen_1$n._)`${evaluated}.items`, items2)
  }
  ...
  class KeywordCxt {
    ...
    fail$data(condition) {
      ...
      this.fail(
        (0, codegen_1$n._)`${schemaCode} !== undefined && (${(0, codegen_1$n.or)(this.invalid$data(), condition)})`
      )
    }
    ...
    invalid$data() {
      ...
      function wrong$DataType() {
        if (schemaType.length) {
          ...
          return (0,
          codegen_1$n._)`${(0, dataType_2.checkDataTypes)(st, schemaCode, it.opts.strictNumbers, dataType_2.DataType.Wrong)}`
        }
  ...
  function getData$1($data, { dataLevel, dataNames, dataPathArr }) {
    ...
    if ($data[0] === '/') {
      if (!JSON_POINTER$2.test($data)) throw new Error(`Invalid JSON-pointer: ${$data}`)
      jsonPointer = $data
  ...
  class MessageChannelClientTransport extends MessageChannelTransport {
    constructor(_0x597360, _0x528781 = getGlobalObject()) {
      ...
      ;((this[_0x1bf448(406)] = _0x2395ac['port1']),
        _0x34395f[_0x1bf448(426)](sendMessage, this[_0x264b2b(401)], { 'endpoint': this[_0x1bf448(391)] }, [
...
`
  ...
  class ExtensionClientTransport {
    // 是否已关闭
    ...
    async start() {
      ...
      try {
        this._messageListener = (message, sender, sendResponse) => {
          ...
          if (message.type === 'mcp-server-to-client') {
            ...
            try {
              ...
              console.error('[ExtensionClientTransport] 处理消息时发生错误:', error2)
              if (this.onerror) {
        ...
        console.error('[ExtensionClientTransport] 启动失败:', error2)
        if (this.onerror) {
          this.onerror(error2 instanceof Error ? error2 : new Error(String(error2)))
        }
        throw error2
    ...
    async send(message, _options) {
      if (!this._isStarted) {
        ...
        console.error('[ExtensionClientTransport]', error2.message)
        if (this.onerror) {
          this.onerror(error2)
        }
        throw error2
      ...
      try {
        chrome.tabs
          ...
          .catch((error2) => {
            if (!error2.message.includes('Receiving end does not exist')) {
              console.error('[ExtensionClientTransport] 发送到标签页', this._tabId, '失败:', error2)
            }
          })
      } catch (error2) {
        console.error('[ExtensionClientTransport] 发送消息失败:', error2)
    ...
    async close() {
      ...
      try {
        ...
        console.error('[ExtensionClientTransport] 关闭时发生错误:', error2)
        if (this.onerror) {
          this.onerror(error2 instanceof Error ? error2 : new Error(String(error2)))
        }
        throw error2
  ...
  class ExtensionServerTransport {
    ...
    _setupMessageListener() {
      const listener = (event) => {
        ...
        if (((_a = event.data) == null ? void 0 : _a.type) === 'sidepanel-ready') {
