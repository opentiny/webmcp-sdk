    ...
    inputSchema: {
      ...
      properties: {
        action: {
          ...
          description: '操作类型：open（打开新网址）、switch（切换标签页）、close（关闭标签页）、switch-pre-tab（切换上一个标签页）、list-tabs（查询全部标签页）'
        },
        url: {
          type: 'string',
      ...
      if (action === 'open') {
        ...
        if (matchedTab && matchedTab.id) {
          try {
            ...
            return { content: [{ type: 'text', text: `网址已在标签页中打开，已切换到该标签页, tabId: ${matchedTab.id}` }] }
          } catch (_) {
