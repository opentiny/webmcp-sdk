...
// --- State ---
export const inventoryList = signal<InventoryItem[]>(initialInventory)
export const priceProtectionList = signal<PriceProtectionOrder[]>(initialPriceProtectionOrders)

// --- Actions ---
export const addInventory = (item: Omit<InventoryItem, 'id' | 'status' | 'lastUpdated'>) => {
  const newId = `INV-${1005 + inventoryList().length}`
...
export const addPriceProtectionOrder = (order: Omit<PriceProtectionOrder, 'id' | 'status' | 'createdAt'>) => {
  const newId = `PP-${new Date().toISOString().replace(/[-:T]/g, '').substring(0, 8)}-${String(priceProtectionList().length + 1).padStart(2, '0')}`
  const now = new Date().toISOString().replace('T', ' ').substring(0, 19)

  priceProtectionList.update((list) => [
    ...list,
    {
      ...order,
      id: newId,
...
export const orderList = signal<OrderItem[]>(initialOrders)
