...
export class PriceProtectionComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    server.registerTool(
      ...
      async ({ id, action, remark }: { id: string | number; action: 'approve' | 'reject'; remark?: string }) => {
        ...
        if (recordId === null) return { content: [{ type: 'text', text: `无效的价保申请 ID：${id}` }] }
  ...
  ngOnDestroy(): void {
    // 组件销毁时取消注册
    server.unregisterTool('price-protection-query')
    server.unregisterTool('price-protection-review')
    server.unregisterTool('price-protection-detail')
  }
