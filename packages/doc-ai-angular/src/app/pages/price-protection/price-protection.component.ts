      ...
      modelContext.registerTool(
        {
          ...
          title: '查询价保申请',
          description:
            '查询商品价保申请列表，可按状态筛选（pending/approved/rejected/expired），不传 status 则返回全部',
          inputSchema: {
            type: 'object',
            properties: {
              status: {
          ...
          execute: async ({ status }: { status?: string }) => {
            const list = status
              ? priceProtectionList().filter(
                  (o: PriceProtectionOrder) => o.status.toLowerCase() === status.toLowerCase()
                )
              : priceProtectionList()
          ...
          execute: async ({
            ...
            return {
              content: [
                {
                  type: 'text',
                  text: `价保申请 ${order.id} 已${action === 'approve' ? '通过' : '拒绝'}${remarkText}。`
                }
              ]
            }
          }
