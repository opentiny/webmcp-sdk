...
export const inputSchema = z.object({
  action: z.enum([
    ...
    .describe(`执行的动作名称, 每一次执行 'click', 'fill', 'select', 'hover'动作之前，**必须**要先调用 'browserState' 去获取页面的最新状态。 
      ...
      clipboard: '读取系统剪切板或复制到剪切板， 依赖 text 参数， text有内容则为写入剪切板，没有内容则为读取剪切板。
      '
  `),
  index: z
    .number()
    .min(0)
    .optional()
    .describe('执行动作 of the element index, 动作为 click,fill,select,hover时，必须提供元素索引'),
  text: z
    .string()
    .optional()
    .describe(
      '执行动作的文本内容, 动作为 fill,select 时，必须提供文本内容；动作为 clipboard时， text有内容则为写入剪切板，没有内容则为读取剪切板'
    ),
