...
Gate Fields / PR Type 勾选对开发者重复劳动。仓库已有约定式标题与 auto-label（`fix`→`bug`，`feat`→`enhancement`）。门禁应：**标题优先定类型 → 标签兜底 → 仅从变更文件校验 Repro/Spec/Feature test**，模板不再要求填写 Gate Fields。

## 目标用户 / 场景

- 开 `fix(...):` PR：变更含至少一个「复现：」测试即可过门禁，无需勾选/填路径
- 开 `feat(...):` PR：变更须同时含至少一个完整 Spec 和至少一个测试文件；琐碎用 label `skip-spec` 仅豁免 Spec
...
2. `feat:` + 变更至少一个完整 Spec 和至少一个 `packages/<pkg>/test/**/*.{test,spec}.*` → OK
3. `feat:` + label `skip-spec` → 仅跳过 Spec 校验，仍须包含至少一个测试文件
...
8. `feat:` 仅有 Spec、没有测试文件 → fail，提示补充 Feature test
