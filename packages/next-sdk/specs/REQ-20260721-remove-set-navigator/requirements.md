...
- 三套示例 + 文档提供可 copy 导航模版（含握手）

### Out of Scope

- 将握手 helper 正式导出为 SDK API
- 改造 page-agent-tool / remoter 核心逻辑（除依赖删除 API 的适配）

## 用户故事与验收标准

1. 作为集成方，我希望自行注册 `navigate_to_page`，以便用业务 router 控制跳转。
   - 验收：SDK 不再导出 `setNavigator` / `registerNavigateTool`；示例可 copy 注册导航工具。
2. 作为集成方，我希望跳转后确认目标页工具已加载完，以便 LLM 可立即调用。
   - 验收：模版使用 `routeToolsMap` + `toolchange` + `getTools` + 轮询超时完成握手。
