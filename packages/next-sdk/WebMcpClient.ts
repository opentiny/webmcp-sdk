          ...
          transport.onmessage = async (message: any) => {
            ...
            try {
              if (message.method === 'initialize') {
                ...
                const nativeCtx = typeof navigator !== 'undefined' ? ((navigator as any).modelContextTesting || (navigator as any).modelContext) : null
                ...
                if (nativeCtx && nativeCtx.executeTool) {
                  ...
                  await transport.send({ jsonrpc: '2.0', id, error: { code: -32601, message: 'Browser built-in WebMCP not available' } })
                }
              }
            } catch (err: any) {
              if (id !== undefined) {
