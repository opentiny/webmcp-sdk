/**
 * 可复制模版：自配路由跳转工具（React + React Router）
 *
 * 用法：
 * 1. 按模块维护 routeToolsMap（path → 该页工具名，工具名全局唯一）
 * 2. initializeBuiltinWebMCP() 之后调用 registerNavigateToPageTool(navigate)
 * 3. 各业务页 useEffect 内 registerTool，工具名与 map 对齐；cleanup 中 abort
 */
...
type NavigateFn = (path: string) => void | Promise<void>

/** 注册 navigate_to_page，供大模型主动跳转并握手等待页面工具就绪 */
export function registerNavigateToPageTool(navigate: NavigateFn): void {
