...
export function useRouteBasedTools(options: {
  /** 是否启用路由感知模式，默认 false */
  enabled: boolean
  /** AgentModelProvider 实例，用于操作 ignoreToolnames */
  agent: any
  ...
  onMounted(() => {
    ...
    // iframe 场景：向父窗口发送 remoter-ready，父窗口会回传 route-state-initial
    if (isInIframe && window.parent) {
      window.parent.postMessage({ type: MSG_REMOTER_READY }, '*')
    }

    // 同窗口：直接全量同步；iframe：等 route-state-initial 后再 sync（或立即 sync 空状态）
    syncAllRoutes()
  })

  // 当 installedPlugins 从空变为有值（MCP 服务器连接完成）时，
  // 重新全量同步一次，确保初始状态正确
  const stopWatch = watch(
    ...
    (len, oldLen) => {
      if (len > 0 && oldLen === 0) {
        ...
        // 解决 iframe 比 Angular 主应用先加载导致第一次握手丢失的问题
